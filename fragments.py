...
BASES = "ACGT"
MAX_INDEX_CANDIDATES = 256
MAX_FILLER_CANDIDATES = 256
...
def _hash_to_dna(seed: str, length: int) -> str:
    ...
    while len(out) < length:
        if i >= len(bits):
            block += 1
            digest = hashlib.sha256((seed + "|" + str(block)).encode("utf-8")).digest()
            bits = []
            for byte in digest:
                for shift in (6, 4, 2, 0):
                    bits.append((byte >> shift) & 0b11)
...
def generate_strand_index(
    ...
    for salt in range(MAX_INDEX_CANDIDATES):
...
def generate_filler(
    ...
    best_seq = ""
    best_score = float("inf")
    for salt in range(MAX_FILLER_CANDIDATES):
