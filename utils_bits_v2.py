...
def detect_magic(data: bytes) -> Optional[MagicInfo]:
    ...
    if head.startswith(b"\xFD7zXZ\x00"):
        return MagicInfo(kind="xz", ext=".xz", mime="application/x-xz", confidence=0.99)

    # BZip2
    if head.startswith(b"BZh"):
        return MagicInfo(kind="bz2", ext=".bz2", mime="application/x-bzip2", confidence=0.99)

    # ZLIB stream: CMF/FLG checksum plus a tiny inflate probe to avoid false positives.
    if len(head) >= 2 and (head[0] & 0x0F) == 8 and ((head[0] << 8) + head[1]) % 31 == 0:
        try:
            zlib.decompressobj(wbits=zlib.MAX_WBITS).decompress(data, 1)
            return MagicInfo(kind="zlib", ext=".zlib", mime="application/zlib", confidence=0.80)
        except Exception:
            pass
...
def _looks_already_compressed(m: Optional[MagicInfo], data: bytes) -> bool:
    if m and m.kind in {"zip","docx","pptx","xlsx","epub","png","jpeg","webp","gzip","xz","bz2","zlib","flac","opus_ogg","ogg","mp4","pdf","mp3","wav","avi","mkv_webm","gif","bmp","tiff"}:
        return True
    if len(data) >= 2048:
        sample = data[:2048]
        uniq = len(set(sample))
